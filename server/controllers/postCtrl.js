...
const postCtrl = {
 ...
 if (!book) {
  book = new Book({
    ...
    author: req.body.book.volumeInfo.authors ? req.body.book.volumeInfo.authors.join(', ') : 'Unknown',
    publicationDate: req.body.book.volumeInfo.publishedDate,
    isbn: req.body.book.volumeInfo.industryIdentifiers ? req.body.book.volumeInfo.industryIdentifiers[0].identifier : 'Unknown',
    description: req.body.book.volumeInfo.description ? req.body.book.volumeInfo.description : "" ,
    genre: req.body.book.volumeInfo.categories ? req.body.book.volumeInfo.categories.join(', ') : 'Unknown',
 ...
 await book.save()  
    ...
    getUserPosts: async (req, res) => {
        try {
            ...
            .paginating()
            const posts = await features.query.sort("-createdAt").populate("book").populate('user').populate({
                path: "comments",
                populate: {
                    path: "user likes",
                    select: "-password"
